...
/** Qwen3-TTS cloud voice with automatic failure callback so native/offline TTS remains the fallback. */
public class QwenTts {
    ...
    private static final String KEY="qwen_api_key";
    ...
    public QwenTts(Context c){context=c.getApplicationContext();prefs="pouya_settings";}
    ...
    public void speak(String text, Callback cb){
        final String key=context.getSharedPreferences(prefs,Context.MODE_PRIVATE).getString(KEY,"").trim();
        if(key.isEmpty()){if(cb!=null)cb.failure("Qwen API key تنظیم نشده");return;}
        if(text==null||text.trim().isEmpty()){if(cb!=null)cb.failure("متن خالی است");return;}
        if(busy){stop();}
        busy=true;
        new Thread(()->{
            HttpURLConnection c=null;
            try{
                JSONObject input=new JSONObject(); input.put("text",text); input.put("voice","Cherry"); input.put("language_type",detectLanguage(text));
                JSONObject body=new JSONObject(); body.put("model",MODEL); body.put("input",input);
                c=(HttpURLConnection)new URL(ENDPOINT).openConnection(); c.setRequestMethod("POST"); c.setConnectTimeout(15000); c.setReadTimeout(60000); c.setDoOutput(true); c.setRequestProperty("Authorization","Bearer "+key); c.setRequestProperty("Content-Type","application/json");
                ...
                if(code<200||code>=300) throw new Exception("Qwen HTTP "+code);
                JSONObject out=new JSONObject(raw).optJSONObject("output"); JSONObject audio=out==null?null:out.optJSONObject("audio"); String audioUrl=audio==null?"":audio.optString("url","");
                if(audioUrl.isEmpty()) throw new Exception("Qwen فایل صوتی برنگرداند");
                c.disconnect(); c=null;
                final String url=audioUrl;
                ((android.app.Activity)context).runOnUiThread(()->play(url,cb));
            }catch(Exception e){busy=false; if(c!=null)c.disconnect(); if(cb!=null)cb.failure(e.getMessage()==null?"Qwen TTS خطا داد":e.getMessage());}
        }).start();
    }
    ...
    private String detectLanguage(String text){
        for(int i=0;i<text.length();i++){char ch=text.charAt(i);if((ch>='\u0600'&&ch<='\u06ff')||(ch>='\u0750'&&ch<='\u077f'))return "Auto";}
        return "English";
    }
